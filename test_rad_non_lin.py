...
from forward.non_lin_rad import NonLinearRadon
...
for snr in [np.linspace(18, 30, 10)[2]]:
    print(snr)
    for realizzazioni in range(25):
        ...
        save_dir = f"results_nat/results_delta_{snr:.1f}/RadonNonLinear/{timestamp}"
        ...
        A = NonLinearRadon(angles, par_cum = par_cum)
        y = A.forward(x)
        ...
        A_lin = LinearRadon(angles)

        x0 = A_lin.adjoint(y_noise, img_shape=(img_size, img_size))
        x0 = x0.clamp(min=0.0)
        x0 = x0 / torch.max(x0)
